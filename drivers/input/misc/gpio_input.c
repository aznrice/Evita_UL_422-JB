...
static void power_key_led_on_work_func(struct work_struct *dummy)
{
	...
	if (power_key_led_requested == 1) {
		...
		printk("[PWR] change power key led on\n");
...
static void power_key_led_off_work_func(struct work_struct *dummy)
{
	if (power_key_led_requested) {
		if (cancel_delayed_work_sync(&power_key_led_on_work)) {
			printk("[PWR] cancel power key led work successfully(%x)\n", power_key_led_requested);
		} else
			printk("[PWR] cancel power key led work unsuccessfully (%x)\n", power_key_led_requested);

		power_key_led_requested = 0;
	}
	if (pre_power_key_led_status == 1) {
		printk("[PWR] change power key led off\n");
...
static void handle_power_key_led(unsigned int code, int value)
{
	if (code == KEY_POWER) {
		...
		if (value) {
			printk("[PWR] start count for power key led on\n");
			schedule_delayed_work(&power_key_led_on_work, PWRKEYLEDON_DELAY);
			power_key_led_requested = 1;
		} else {
			printk("[PWR] start count for power key led off\n");
...
static void power_key_clr_check_work_func(struct work_struct *dummy)
{
	if (cancel_delayed_work_sync(&power_key_check_reset_work))
		printk("[PWR] cancel power key check reset work successfully\n");
	else
		printk("[PWR] cancel power key check reset work unsuccessfully\n");
...
static void handle_power_key_reset(unsigned int code, int value)
{
	if (code == KEY_POWER) {
		if (value) {
			...
			printk("[PWR] start count for power key check reset\n");
			if (!schedule_delayed_work(&power_key_check_reset_work, PWRKEYCHKRST_DELAY))
				printk("[PWR] the reset work in already in the queue\n");
		} else {
			printk("[PWR] start count for power key clear check\n");

			if (!schedule_delayed_work(&power_key_clr_check_work, PWRKEYCLRCHK_DELAY))
				printk("[PWR] the clear work in already in the queue\n");
		}
	}
}
#endif
	...
	for (i = 0; i < nkeys; i++, key_entry++, key_state++) {
		...
		if (key_state->debounce & DEBOUNCE_UNSTABLE) {
			...
			if (gpio_flags & GPIOEDF_PRINT_KEY_UNSTABLE)
				printk("gpio_keys_scan_keys: key %x-%x, %d "
					"(%d) continue debounce\n",
		...
		if (debounce & DEBOUNCE_POLL) {
			if (pressed == !(debounce & DEBOUNCE_PRESSED)) {
				...
				if (gpio_flags & GPIOEDF_PRINT_KEY_DEBOUNCE)
					printk("gpio_keys_scan_keys: key %x-"
...
static int gpio_event_input_request_irqs(struct gpio_input_state *ds)
{
	...
	for (i = 0; i < ds->info->keymap_size; i++) {
		...
		if (ds->info->keymap[i].code == KEY_POWER) {
			...
			printk("keycode = %d, gpio = %d, type = %lx", ds->info->keymap[i].code, value, req_flags);
		}
		else
		...
		if (err < 0) {
			pr_err("gpio_event_input_func: %s: request_irq "
		...
		if (ds->info->keymap[i].code == KEY_VOLUMEUP ||
			...
			printk("keycode = %d, gpio = %d, irq = %d", ds->info->keymap[i].code, ds->info->keymap[i].gpio, irq);
			if (ds->info->keymap[i].code == KEY_VOLUMEUP)
				vol_up_irq = irq;
			else
				vol_down_irq = irq;
			...
			struct gpio_event_info *info, void **data, int func)
{
	...
	if (func == GPIO_EVENT_FUNC_INIT) {
		...
		*data = ds = kzalloc(sizeof(*ds) + sizeof(ds->key_state[0]) *
		...
		if (ds == NULL) {
			ret = -ENOMEM;
			pr_err("gpio_event_input_func: %s: "
		...
		for (i = 0; i < di->keymap_size; i++) {
			...
			if (dev >= input_devs->count) {
				pr_err("gpio_event_input_func: %s: bad device "
		...
		for (i = 0; i < di->keymap_size; i++) {
			...
			if (ret) {
				pr_err("gpio_event_input_func: %s: gpio_request "
		...
		if (keyboard_kobj == NULL) {
			pr_err("gpio_event_input_func: %s: subsystem_register failed\n", __func__);
			ret = -ENOMEM;
			return ret;
		}
		if (sysfs_create_file(keyboard_kobj, &dev_attr_vol_wakeup.attr))
			pr_err("gpio_event_input_func: %s: sysfs_create_file "
					"return %d\n", __func__, ret);
		wakeup_bitmask = 0;
		set_wakeup = 0;
		spin_lock_irqsave(&ds->irq_lock, irqflags);
		...
		printk("GPIO Input Driver: Start gpio inputs for %s%s in %s "
